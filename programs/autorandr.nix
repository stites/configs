{ lib, pkgs, ... }:
let
  bigboi-fingerprint = lib.strings.concatStringsSep "" [
  ...
  # lab setup -- currently dynamic as I figure out issues with docks:
  labBuilder = lab-port: {
    fingerprint = laptop-monitor //
      { "${lab-port}" = bigboi-fingerprint; };
    config = {
      eDP1 = laptop-defaults // {
        enable = true;
        primary = false;
        position = "630x1600";
      };
      "${lab-port}" = external-defaults // {
        mode = "3840x1600";
      };
    };
  };
...
in
{
  programs.autorandr = {
    ...
    profiles = {
      ...
      lab = labBuilder "HDMI1";
      lab-tb1 = labBuilder "DP1-1";
      lab-tb2 = labBuilder "DP2-1";
    };
  };
}
